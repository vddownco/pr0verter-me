...
const setErrors = watch(() => props.errors, (newValue) => {
    if (newValue) {
        for (let error in newValue) {
            form.setErrors({
...
const formSchema = toTypedSchema(z.object({
    ...
    audioQuality: z.number().min(0.01).max(1.00, 'Die Zahl muss zwischen 1 und 100 liegen.').nonnegative().default(1.00),
    ...
    toast.promise(submitPromise, {
        loading: computed(() => {
            if (inertiaForm.file && inertiaForm.processing) {
                return 'Datei wird hochgeladen (' + inertiaForm.progress?.percentage + '%) und Konvertierung gestartet'
            }
...
<template>
    <form class="grid w-full items-start gap-6" @submit.prevent="onSubmit">
        ...
        <fieldset class="grid gap-6 rounded-lg border p-4">
            ...
            <Tabs default-value="upload" class="w-full">
                <TabsList class="w-full">
                    <TabsTrigger value="upload" class="w-full">
                        Datei hochladen
                    </TabsTrigger>
                    <TabsTrigger value="download" class="w-full">
                        Download
                    </TabsTrigger>
                </TabsList>
                <TabsContent value="upload">
                    <FormField v-slot="{ value, handleChange, handleBlur }" name="file">
                        <FormItem class="flex h-60 shrink-0 items-center rounded-md border relative"
                                  :class="{'border-dashed': !isOverDropZone, 'border-primary': isOverDropZone, 'justify-center': !form.values.file}"
                                  ref="dropZoneRef">
                            <label for="file-input"
                                   :class="{'absolute inset-0': !form.values.file, 'hidden': form.values.file}"
                                   class=" w-full h-full cursor-pointer rounded-md"></label>
                            <div v-show="!form.values.file"
                            ...
                            <div v-if="form.values.file"
                                 class="flex lg:flex-row flex-col justify-between rounded-lg border p-4 w-full m-4 self-start lg:items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <div class="flex flex-row gap-x-4 items-center">
                                        <span class="text-muted-foreground">{{
                                                sizeOfFile(form.values.file).toFixed(2)
                                            }} MB</span>
                                    </div>
                                </div>
                                <div>
                            ...
                            <FormControl>
                                <Input type="file" id="file-input" class="sr-only" accept="video/*"
                                       @change="handleChange"
                                       @blur="handleBlur"/>
                            </FormControl>
                        </FormItem>
                        ...
                        <div class="flex h-60 shrink-0 p-4 rounded-md border">
                            <div class="flex flex-col w-full">
                                <div class="space-y-1">
                                    ...
                                    <Input id="url" :model-value="value" @update:modelValue="handleChange"
                                           class="w-full"/>
                                    <p class="text-sm text-muted-foreground">Kann alles sein. YouTube, einfache Datei
                                        oder
                                        sonst was</p>
                                </div>
                            </div>
        ...
        <fieldset class="grid gap-6 rounded-lg border p-4">
            ...
            <FormField v-slot="{ value, handleChange }" name="audioQuality" v-if="form.values.audio === true">
                <Label for="audioQuality">
                    <FormItem class="flex flex-row items-center justify-between rounded-lg border p-4">
                        ...
                        <FormControl>
                            <div class="flex flex-row gap-x-4 items-center">
                                <RotateCcw class="text-muted-foreground size-4 cursor-pointer"
      ...
      style: 'percent',
            ...
            <FormField v-slot="{ value, handleChange }" name="autoCrop">
                <label for="autoCrop" class="cursor-pointer">
                    <FormItem class="flex flex-row items-center justify-between rounded-lg border p-4">
                        <div class="space-y-0.5">
                            <FormLabel class="text-base">
                            ...
                            <FormDescription>
                                Cropt deinen Unrat automatisch. Nutzt Bewegungsvektoren des Decoders um den Videobereich
                                zu erkennen.<br/>
            ...
            <FormField v-slot="{ value, handleChange }" name="watermark">
                <label for="watermark" class="cursor-pointer">
                    <FormItem class="flex flex-row items-center justify-between rounded-lg border p-4">
                        <div class="space-y-0.5">
                            <FormLabel class="text-base">
                                Wasserzeichen
